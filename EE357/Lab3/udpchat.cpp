...
int main(int argc, char *argv[])
{
    ...
    {                                      // talker
        ...
        {
            if ((rv = getaddrinfo(network_ips[host_idx], SERVERPORT, &hints, &(servinfo[host_idx]))) != 0)
            {
                fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(rv));
                return 1;
            }
            ...
            // loop through all the results and make a socket
            for (p[host_idx] = servinfo[host_idx]; p[host_idx] != NULL; p[host_idx] = p[host_idx]->ai_next)
            {
                if ((talkfd[host_idx] = socket(p[host_idx]->ai_family, p[host_idx]->ai_socktype,
